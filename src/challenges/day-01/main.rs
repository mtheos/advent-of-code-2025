...
fn run_easy(input: &Vec<Sequence>, mut dial_position: i32, dial_limit: i32) -> Answer {
  ...
  Answer { dial_position, zero_count }
}
...
mod tests {
  ...
  fn test_sample_input_1 () {
    let raw_input = vec!["L68", "L30", "R48", "L5", "R60", "L55", "L1", "L99", "R14", "L82"];
    let parser = DialParser { dial_limit: 99 };
    let input = raw_input.iter().map(|x| parser.parse(x)).collect::<Vec<Sequence>>();
    let result = run_easy(&input, 50, 99);
  ...
  #[test]
  fn test_easy_1 () {
    let input = vec![Sequence { rollovers: 0, magnitude: 15 }];
    let result = run_easy(&input, 50, 99);
  ...
  #[test]
  fn test_easy_2 () {
    let input = vec![Sequence { rollovers: 0, magnitude: 15 }, Sequence { rollovers: 0, magnitude: 35 }, Sequence { rollovers: 0, magnitude: 40 }];
    let result = run_easy(&input, 50, 99);
  ...
  #[test]
  fn test_easy_3 () {
    let input = vec![Sequence { rollovers: 0, magnitude: -60 }];
    let result = run_easy(&input, 50, 99);
  ...
  #[test]
  fn test_hard_2 () {
    let input = vec![Sequence { rollovers: 0, magnitude: 15 }, Sequence { rollovers: 0, magnitude: 30 }, Sequence { rollovers: 0, magnitude: 45 }];
    let result = run_hard(&input, 50, 99);
    assert_eq!(result.dial_position, 40);
    assert_eq!(result.zero_count, 1);
  }
  ...
  #[test]
  fn test_hard_4 () {
    let input = vec![Sequence { rollovers: 0, magnitude: 500 }];
    let result = run_hard(&input, 50, 99);
    assert_eq!(result.dial_position, 50);
    assert_eq!(result.zero_count, 5);
  }
}